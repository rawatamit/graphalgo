...
#define EPSILON 0.01
using namespace std;
//void accumulate_basic(float* betweenness, row &S,tablist &P,int* sigma, int s)

template<typename T>
void accumulate_basic(Graph<T> const& g, float* betweenness, vector<unsigned>* S,ShortestPaths const* &sp_karger,std::vector<unsigned>* E, int s)
{
    /*
  ...
  for(unsigned v=0; v < numV; ++v)
  {
    for(auto u : E[v])
    {
      ...
      if(weightSU != numeric_limits<double>::infinity() and weightSV!=numeric_limits<double>::infinity())
      {
        double diff=abs(weightSU+edgeWeight-weightSV);
        if(diff<=EPSILON)
        {
          P[v].push_back(u);			
        }
  ...
  {   
    ...
    for(vector<int>::iterator it=P[w].begin();it!=P[w].end();++it)
    {
        int v=*it;
